    ...
    const onSubmitHandler = async (e) => {
        try {
            ...
            const { data } = await axios.post(`/api/hotels/`,
                { name, address, contact, city }, { headers: { Authorization: `Bearer ${await getToken()}` } })
            if (data.success) {
                ...
                <div className='relative flex flex-col items-center md:w-1/2 p-8 md:p-10'>
                    ...
                    <div className='w-full mt-4'>
                        ...
                        <input id='name' onChange={(e) => setName(e.target.value)} value={name}
                            type="text" placeholder='Type here' className='border
                    ...
                    <div className='w-full mt-4'>
                        ...
                        <input id='contact' onChange={(e) => setContact(e.target.value)} value={contact}
                            type="text" placeholder='Type here' className='border
                        border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500
                        font-light ' required />
                    </div>
